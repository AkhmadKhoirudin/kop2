<?php 
    ...
    ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    // 3. Ambil data denda dengan filter bulan
        $query_denda = "SELECT rd.*, a.nama, p.jumlah as jumlah_pinjaman, an.tanggal as tanggal_angsuran
                    FROM riwayat_denda rd
                    JOIN angsuran an ON rd.id_angsuran = an.id_angsuran
                    JOIN pinjaman p ON an.id_pinjaman = p.id_pinjaman
                    JOIN anggota a ON p.id_anggota = a.id_anggota
                    WHERE rd.tanggal BETWEEN '$tanggal_awal' AND '$tanggal_akhir'
                    ORDER BY rd.tanggal DESC";
            $result_denda = mysqli_query($conn, $query_denda);
            if (!$result_denda) {
                die("Error dalam query denda: " . mysqli_error($conn));
            }
            $denda_data = [];
    ...
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    // 5. Ambil Saldo Awal Bulan Sebelumnya
    ...
    // 6. Total Simpanan untuk bulan ini
    ...
    // 7. Total Angsuran untuk bulan ini
    ...
    // 8. Total Denda untuk bulan ini
        $query_denda = "SELECT SUM(jumlah) as total FROM riwayat_denda WHERE tanggal BETWEEN '$tanggal_awal' AND '$tanggal_akhir'";
        $result_denda = mysqli_query($conn, $query_denda);
        $total_denda = mysqli_fetch_assoc($result_denda)['total'] ?? 0;

    // 9. Hitung Saldo Akhir
        // $saldo_akhir = $saldo_sebelum + $total_simpanan + $total_angsuran + $total_denda;   //seharusnya yang ini 
            $saldo_akhir = $saldo_sebelum + $total_simpanan + $total_angsuran ;

    // 10 Nama bulan untuk ditampilkan
    $nama_bulan = [
        1 => 'Januari',
        2 => 'Februari',
        3 => 'Maret',
...
<head>
    ...
    <style>
        ...
        @media print {
            @page {
                size: A4;
                margin: 1cm; /* Atur margin halaman */
                /* Coba nonaktifkan header/footer */
                margin-top: 1cm;
                margin-bottom: 1cm;
                marks: none; 
            ...
            body {
                -webkit-print-color-adjust: exact; /* Memastikan warna dicetak dengan benar */
                margin: 0;
                padding: 0;
                background: none;
            }
            /* Sembunyikan elemen yang mungkin ditambahkan browser */
            body::before,
            body::after {
                display: none !important;
            }
            /* Sembunyikan header dan footer browser */
            header,
            footer {
                display: none !important;
            }
            ...
            .container {
                ...
                padding: 20px; /* Pertahankan padding */
            ...
            .print-button, .filter-form {
                display: none; /* Sembunyikan tombol cetak dan form filter saat print */
            }

            /* Pastikan tidak ada elemen tambahan dari browser */
            * {
...
<body>
    ...
    <div class="container">
        <div class="kop">
            <div class="kop-header">
                ...
                <img src="../logo.png" alt="Logo Koperasi Kanan" class="logo-kanan">
            </div>
            <br>
            <div class="line"></div>
        ...
        <ul>     
            ...
            <!-- <li>Denda/Keterlambatan: Rp <?//= number_format($total_denda, 0, ',', '.') ?></li> -->
        ...
        <table class="ck-table-resized">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><?= htmlspecialchars($simpanan['jenis']) ?></td>
 ...
 <!--  \////////////////2 /////////////////////////////// -->
        ...
        <table class="ck-table-resized">
            ...
            <tbody>
                ...
                $no = 1;
                foreach ($pinjaman_data as $pinjaman): 
                    // Hitung angsuran per bulan (contoh: jumlah dibagi tenor)
                ...
                <tr>
                    ...
                    <td><?= htmlspecialchars($pinjaman['akad']) ?></td>
 ...
 <!--     \/////////////5 //////////////////////////// -->
        ...
        <!-- <br><h2 class="section-title">5. </h2> -->
        <table class="ck-table-resized">
            <thead>
                <tr>
                    <th>No</th>
            ...
            <tbody>
                ...
                $evaluasi_data = [
                    ...
                    // [
                    //     'bidang' => 'Denda',
                    //     'bulan_lalu' => getTotalDendaBulanLalu($conn, $bulan, $tahun),
                    //     'bulan_kini' => $total_denda,
                    //     'evaluasi' => '',
                    //     'keterangan' => ''
                    // ],
                    [
                ...
                // function getTotalDendaBulanLalu($conn, $bulan, $tahun) {
                //     $bulan_lalu = $bulan - 1;
                //     $tahun_lalu = $tahun;
                //     if ($bulan_lalu < 1) {
                //         $bulan_lalu = 12;
                ...
                //     $query = "SELECT SUM(jumlah) as total FROM riwayat_denda WHERE tanggal BETWEEN '$tanggal_awal' AND '$tanggal_akhir'";
                //     $result = mysqli_query($conn, $query);
                //     $row = mysqli_fetch_assoc($result);
                //     return $row['total'] ?? 0;
                // }
