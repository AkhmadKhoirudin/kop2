<?php
session_start();
require_once '../config/database.php'; // Sesuaikan path sesuai struktur folder Anda
...
if (!in_array($jenis_transaksi, ['simpanan', 'pinjaman', 'tarik'])) {
...
switch ($jenis_transaksi) {
    case 'simpanan':
        $query = "
            SELECT s.id_simpanan, s.id_anggota, s.tanggal, s.jumlah, p.nama as jenis, 
    ...
    case 'pinjaman':
        $query = "
            SELECT p.id_pinjaman, p.id_anggota, p.tanggal_pengajuan, p.jumlah, p.tenor, 
                   pr.nama as akad, a.nama, a.alamat, a.telepon
            FROM pinjaman p
    ...
    case 'tarik':
        $query = "
            SELECT t.id_tarik, t.id_anggota, t.tanggal, t.jumlah, p.nama as jenis, 
                   a.nama, a.alamat, a.telepon
            FROM tarik t
...
<div class="struk">
    <div class="header">
        <img src="../logo.png" alt="Koperasi" class="logo logo-kiri">
